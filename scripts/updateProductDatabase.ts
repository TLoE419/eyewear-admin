...
async function updateProductDatabase() {
  console.log("🔄 開始更新產品資料庫...");

  try {
    // 1. 首先創建 products 表（如果不存在）
    console.log("📋 檢查 products 表是否存在...");
    ...
    if (photosError) {
      throw new Error(`獲取照片失敗: ${photosError.message}`);
    }
    ...
    for (const product of products) {
      // 找到對應的照片（根據 display_order 匹配產品 ID）
    ...
    console.log("\n🎉 產品資料庫更新完成！");
    console.log("\n📋 下一步:");
    console.log("1. 檢查 Supabase 中的 products 表");
    console.log("2. 更新應用程式使用 products 表而不是 JSON 文件");
    console.log("3. 測試產品管理功能");
  } catch (error) {
