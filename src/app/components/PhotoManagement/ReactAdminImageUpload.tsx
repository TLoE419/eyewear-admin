...
interface ReactAdminImageUploadProps extends Omit<InputProps, "source"> {
  source: string;
  category?: PhotoCategory;
  onReplaceImage?: (oldUrl: string, newUrl: string) => Promise<void>;
}

export const ReactAdminImageUpload: React.FC<ReactAdminImageUploadProps> = ({
  source,
  category = "brand_logo",
  onReplaceImage,
  ...props
  ...
  const handleFileChange = async (
    ...
    try {
      setLoading(true);
      const oldUrl = field.value; // 保存舊的圖片 URL
      ...
      field.onChange(newImageUrl); // 使用 React Admin 的 field.onChange
    } catch (err) {
  ...
  const handleUrlChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPreviewUrl(url);
    field.onChange(url); // 使用 React Admin 的 field.onChange
  };

  const currentImageUrl = previewUrl || field.value;
